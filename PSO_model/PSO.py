...
class PSO(SkoBase):
    ...
    def cal_y(self):
        ...
        optionsList = [self.options] * len(self.X)

        self.Y = self.func(self.X,optionsList).reshape(-1, 1)
        return self.Y
    ...
    def run(self, max_iter=None, precision=1e-7, N=20,logger=None):
        '''
        ...
        for iter_num in tqdm(range(self.max_iter)):
            print("*"*40+"iter_num--"+str(iter_num)+"--"+"*"*40)
            self.update_V()
            self.recorder()
            self.update_X()
            self.cal_y()
            ...
            if precision is not None:
                ...
                if tor_iter < precision:
                    c = c + 1
                    if c > N:
                        logger.info("-------------Approving the PSO convergence precision {}, stop the PSO optimal model.----------".format(precision))
                        break
                else:
                    c = 0
            if self.verbose:
            ...
            self.gbest_FMI = 1 / self.gbest_y - 1
            self.gbest_x_t = np.round(self.gbest_x, 1)

            logger.info(localtime+"--PSO--iter:"+str(iter_num)+"----gobal_to_now_best_x:"+str(self.gbest_x_t)+"---gobal_to_now_best_FMI: "+str(self.gbest_FMI))
            # logger.info(localtime+"--PSO--iter:"+str(iter_num)+"---iter_Y: "+str(self.pbest_y[idx_min])+"---gobal_to_now_best_y: "+str(self.gbest_y))
            # logger.info(localtime+"--PSO--iter:"+str(iter_num)+" iter_Y: "+str(self.gbest_y)+" iter_X: "+str(self.gbest_x))

        self.best_x, self.best_FMI = self.gbest_x_t, self.gbest_FMI
        return self.best_x, self.best_FMI
